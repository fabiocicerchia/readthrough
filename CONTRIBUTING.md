...
The workflow runs release-please twice, and the order matters. Repos that cut a
*draft* release — the only way to attach assets, since releases are immutable —
have no tag until the draft is published, so computing the next release PR in
the same invocation leaves release-please with no anchor: it walks the whole
history and opens a PR re-releasing everything. Phase 1 (`release-please`) cuts
the release, the jobs in between attach assets and publish the draft, and phase
2 (`release-pr`) computes the next release PR once the tag exists.
